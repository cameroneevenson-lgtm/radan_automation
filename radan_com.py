...
class _Win32ComBackend(_Backend):
    ...
    def __init__(self, prog_id: str, create_if_missing: bool = True) -> None:
...
def _make_backend(prog_id: str, backend: str | None = None, create_if_missing: bool = True) -> _Backend:
    backend_factories = {
    ...
    if backend is not None:
        ...
        return backend_factories[backend](prog_id, create_if_missing=create_if_missing)

    errors: list[str] = []
    for candidate in ("win32com", "comtypes", "powershell"):
        factory = backend_factories[candidate]
        try:
            return factory(prog_id, create_if_missing=create_if_missing)
...
class RadanApplication:
    ...
    def __init__(
        ...
        self._backend = _make_backend(prog_id, backend=backend, create_if_missing=create_if_missing)
...
def open_application(backend: str | None = None, create_if_missing: bool = True) -> RadanApplication:
    return RadanApplication(backend=backend, create_if_missing=create_if_missing)
...
def probe_application(backend: str | None = None) -> RadanApplicationInfo:
    with open_application(backend=backend) as app:
        info = app.info()
