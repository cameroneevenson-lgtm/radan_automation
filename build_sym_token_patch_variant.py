...
def parse_row_patch_spec(value: str) -> dict[str, Any]:
    fields = str(value).split(":")
    if len(fields) != 2:
        raise ValueError(f"Row patch specs must be PART:ROW, got: {value!r}")
    part, row_text = fields
    if not part.strip():
        raise ValueError(f"Row patch spec has an empty part name: {value!r}")
    row_index = int(row_text)
    if row_index <= 0:
        raise ValueError(f"Row patch index must be 1-based and positive: {value!r}")
    return {"part": part.strip(), "row_index": row_index, "source": "row-spec"}
...
def _source_geometry_line(source_folder: Path, patch: dict[str, Any]) -> tuple[str, str]:
    rows = read_ddc_records(source_folder / f"{patch['part']}.sym")
    row_index = int(patch["row_index"])
    if row_index > len(rows):
        raise RuntimeError(f"Source {patch['part']} has no geometry row {row_index}.")
    row = rows[row_index - 1]
    return str(row.get("raw", "")), str(row.get("record", ""))


def _replace_geometry_row(text: str, patch: dict[str, Any], raw_line: str) -> tuple[str, dict[str, Any]]:
...
def build_token_patch_variant(
    *,
    ...
    grouped_row_patches: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for patch in row_patches or []:
        grouped_row_patches[str(patch["part"])].append(patch)

    patch_results: list[dict[str, Any]] = []
    field_patch_results: list[dict[str, Any]] = []
    row_patch_results: list[dict[str, Any]] = []
    patched_parts = set(grouped) | set(grouped_field_patches) | set(grouped_row_patches)
    for part in sorted(patched_parts, key=str.casefold):
        ...
        part_row_patches = grouped_row_patches.get(part, [])
        target_path = out_dir / f"{part}.sym"
        if not target_path.exists():
            raise RuntimeError(f"Target symbol not found in base copy: {target_path}")
        text = target_path.read_text(encoding="utf-8", errors="replace")
        for patch in sorted(part_row_patches, key=lambda row: int(row["row_index"])):
            raw_line, source_record = _source_geometry_line(source_folder, patch)
            text, result = _replace_geometry_row(text, patch, raw_line)
            result["source_record"] = source_record
            row_patch_results.append(result)
    ...
    manifest = {
        ...
        "row_patch_count": len(row_patch_results),
        "changed_row_patch_count": sum(1 for row in row_patch_results if row["changed"]),
        "patched_parts": sorted(patched_parts, key=str.casefold),
        "patches": patch_results,
        "field_patches": field_patch_results,
        "row_patches": row_patch_results,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--row-patch", action="append", default=[], help="Row patch spec PART:ROW.")
    ...
    row_patches = [parse_row_patch_spec(value) for value in args.row_patch]
    ...
    if not patches and not field_patches and not row_patches:
        raise RuntimeError("No token, field, or row patches requested.")
