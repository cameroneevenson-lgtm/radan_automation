...
def _pad_decoded_close_zeros(token: str, *, min_length: int, tolerance: float = 1e-12) -> str:
    if not token or len(token) >= int(min_length):
        return token
    candidate = token + ("0" * (int(min_length) - len(token)))
    try:
        diff = abs(float(decode_ddc_number_fraction(candidate) - decode_ddc_number_fraction(token)))
    except Exception:
        return token
...
def _repair_h_delta_zero_tokens(tokens: list[str]) -> list[str]:
    repaired = list(tokens)
    for index in (2, 3, 4, 5):
        if index < len(repaired):
            repaired[index] = _pad_decoded_close_zeros(repaired[index], min_length=11)
    return repaired
...
def encode_geometry_data(
    ...
    *,
    ...
    h_delta_repair_zero: bool = False,
) -> str:
    ...
    if entity_type == "CIRCLE":
        ...
        if canonicalize_endpoints:
            ...
            padded = _pad_fraction_tokens(tokens, token_count, continuation_digits=continuation_digits)
            if h_delta_repair_zero:
                padded = _repair_h_delta_zero_tokens(padded)
            return ".".join(padded)
        tokens = {
    ...
    def generated_geometry_lines() -> list[str]:
        ...
        for index, dxf_row in enumerate(dxf_rows, start=3):
            ...
            geometry_data = encode_geometry_data(
                ...
                h_delta_repair_zero=h_delta_repair_zero,
            )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Lab option: pad decoded-close H delta/center-offset tokens with trailing zeros."
        ),
    )
    parser.add_argument(
    ...
    payload = write_native_prototype(
        ...
        h_delta_repair_zero=bool(args.h_delta_repair_zero),
